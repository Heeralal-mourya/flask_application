const init = function() {
  document
    ...
    .addEventListener("click", revenueslider);
};
...
var revenueg = revenuesvg.append("g");

var revenuetooltip = d3
  .select("#revenuemap")
  .append("div")
  .attr("class", "tooltip")
  .style("opacity", 1)
...
function redraw(year) {
  d3.json("json-data/worldmap.json", function(error, topology) {
    ...
    d3.json("http://localhost:9007/ong/revenuemapchart")
      ...
      .get(function(error, data) {
        ...
        var revlabel = revenueg.selectAll("text").data(
          sorted.filter(function(d) {
            return d.year == year;
          })
        );
        revlabel.enter().append("svg:text");
        revlabel.exit().remove();
        revlabel
          .attr("class", "label")
          .transition()
          .duration(1000)
