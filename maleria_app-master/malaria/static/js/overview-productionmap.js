const init = function() {
  document.getElementById("year_slider").addEventListener("click", prodslider);
};
...
var productionsvg = d3
  .select("#productionmap")
  .append("svg")
...
var productiong = productionsvg.append("g");

var productiontooltip = d3
  .select("#productionmap")
  .append("div")
  .attr("class", "tooltip")
  .style("opacity", 1)
...
var labels = productionsvg.append("svg:g").attr("id", "labels");

function redraw(year) {
  d3.json("json-data/worldmap.json", function(error, topology) {
    // load and display the cities
    d3.json("http://localhost:9007/ong/productionmapchart")
      ...
      .get(function(error, data) {
        ...
        var prodlabel = productiong.selectAll("text").data(
          sorted.filter(function(d) {
            return d.year == year;
          })
        );
        prodlabel.enter().append("svg:text");
        prodlabel.exit().remove();
        prodlabel
          .attr("class", "label")
          .transition()
          .duration(1000)
