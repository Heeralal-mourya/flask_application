const init = function() {
  document
    ...
    .addEventListener("click", importslider);
};
...
var importg = importsvg.append("g");

var importtooltip = d3
  .select("#importmap")
  .append("div")
  .attr("class", "tooltip")
  .style("opacity", 1)
...
function redraw(year) {
  d3.json("json-data/worldmap.json", function(error, topology) {
    ...
    d3.json("http://localhost:9007/ong/importmapchart")
      ...
      .get(function(error, data) {
        ...
        var impolabel = importg.selectAll("text").data(
          sorted.filter(function(d) {
            return d.year == year;
          })
        );
        impolabel.enter().append("svg:text");
        impolabel.exit().remove();
        impolabel
          .attr("class", "label")
          .transition()
          .duration(1000)
